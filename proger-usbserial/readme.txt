To setup the project and upload the Proger usbserial application firmware to an ATMEGA32U2 using the Arduino USB DFU bootloader:
1. unpack the source into LUFA's Projects directory
2. set PROGER_MODEL_PID in the makefile as appropriate
...
5. confirm that the board enumerates as "Proger DFU"
6. do "make dfu" (OS X or Linux - dfu-programmer must be installed first) or "make flip" (Windows - Flip must be installed first)

Check that the board enumerates as "Proger".  Test by uploading a new Arduino sketch from the Arduino IDE.
